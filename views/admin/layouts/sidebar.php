<?php
$baseUrl = "/MiniShop_HoThiBichNhung";
?>
<div class="col-md-3 col-lg-2 bg-dark text-white min-vh-100 p-3">
    <a href="#" class="d-flex align-items-center mb-4 text-white text-decoration-none fs-5 fw-bold px-2">
        <i class="bi bi-cart3 fs-4 me-2"></i> Mini Shop
    </a>

    <ul class="nav nav-pills flex-column mb-auto gap-1">
        <li class="nav-item">
            <a href="<?= $baseUrl ?>/admin/dashboard" class="nav-link text-white-50 py-2 px-3">
                <i class="bi bi-house-door-fill me-3"></i> Dashboard
            </a>
        </li>
        <li>
            <a href="<?= $baseUrl ?>/admin/category" class="nav-link text-white-50 py-2 px-3">
                <i class="bi bi-folder2-open me-3"></i> Danh mục
            </a>
        </li>
        <li>
            <a href="<?= $baseUrl ?>/admin/brand" class="nav-link text-white-50 py-2 px-3">
                <i class="bi bi-star me-3"></i> Thương hiệu
            </a>
        </li>
        <li>
            <a href="<?= $baseUrl ?>/admin/product" class="nav-link text-white-50 py-2 px-3">
                <i class="bi bi-bag me-3"></i> Sản phẩm
            </a>
        </li>
        <li>
            <a href="<?= $baseUrl ?>/admin/customer" class="nav-link text-white-50 py-2 px-3">
                <i class="bi bi-people me-3"></i> Khách hàng
            </a>
        </li>
        <li>
            <a href="<?= $baseUrl ?>/admin/user" class="nav-link text-white-50 py-2 px-3">
                <i class="bi bi-person me-3"></i> Người dùng
            </a>
        </li>
        <li>
            <a href="<?= $baseUrl ?>/admin/order" class="nav-link text-white-50 py-2 px-3">
                <i class="bi bi-clipboard-data me-3"></i> Đơn hàng
            </a>
        </li>
    </ul>
</div>